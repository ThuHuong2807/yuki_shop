...
class slider extends MY_Controller {
		...
		if ($this->input->post()) {
			...
			if ($this->form_validation->run()) {
				...
				if ($this->slider_model->create($data)) {
					...
					$this->session->set_flashdata('message_fail', 'Add slider failed');
				}
		...
		if ($this->input->post()) {
			...
			if ($this->form_validation->run()) {
				...
				if ($this->slider_model->update($id,$data)) {
					...
					$this->session->set_flashdata('message_fail', 'Change slider failed');
				}
				redirect(admin_url('slider'));
			}
		}
	...
	public function del()
	{
		...
		if ($this->slider_model->delete($id)) {
			...
			$this->session->set_flashdata('message_fail', 'Slider deletion failed');
		}
		redirect(admin_url('slider'));
	}
}
