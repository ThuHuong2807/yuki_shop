	...
	public function index()
	{
		...
		if ($this->input->post())
		{
			...
			if ($flag > 0) {
				...
				$this->session->set_flashdata('message_fail', 'Delete failed products');
			}
		...
		if ($this->input->post()) {
			...
			if ($this->form_validation->run()) {
				...
				if ($this->product_model->create($data)) {
					$this->session->set_flashdata('message_success', 'Successful');
				}else{
					$this->session->set_flashdata('message_fail', 'Add failed products');
				}
				redirect(admin_url('product'));
			}
		}
		...
		if ($this->input->post()) {
			...
			if ($this->form_validation->run()) {
				...
				if ($this->product_model->update($id,$data)) {
					...
					$this->session->set_flashdata('message_fail', 'Change failed product');
				}
				redirect(admin_url('product'));
			}
		}
